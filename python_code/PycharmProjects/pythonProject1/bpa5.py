...
def processdata(imsorgid,custname,contractenddate):
    print(imsorgid)
    for ims in imsorgid:
        if (not ims):
            continue
        ...
        try:
            ...
            if customerdata["reports"][0]["overview"]["assetCount"] > noofassets:
                ...
                continue
        print(custname[indx], " :- ", ims, "=", customerdata["reports"][0]["overview"]["overallComplexity"])
        ...
        elif customerdata["reports"][0]["overview"]["overallComplexity"] > 1.15 and \
                customerdata["reports"][0]["overview"]["overallComplexity"] <= 1.61:
            LowMediumcust.append(custname[indx])
            if d4 > d3:
                lowhangingfruits.append(custname[indx])
        elif customerdata["reports"][0]["overview"]["overallComplexity"] > 1.61 and \
                customerdata["reports"][0]["overview"]["overallComplexity"] <= 2.07:
...
def displaystats():
    ...
    x = ["Low", "Low Medium", "Medium", "MediumHigh", "High", "ExtremelyHigh", "Not in CAM Database",
         "Currently Blocked"]
    ...
    if ans == "y":
        plt.ylabel("Basic All Customers")
    else:
    ...
    print("Customers which are Blocked: ", "High Node Store Size - ", custblocknodestoresize, "High JCR Nodes - ",
          custblockjcrnodes, "High No of Assets - ", custblocknoofassets)
...
def exportcsv():
    path = r'/Users/meejain/Downloads/BPA Report rerun.csv'
    data = {"BPA Report needs to be rerun": nodata}
    df = pd.DataFrame(data).to_csv(path)
    path = r'/Users/meejain/Downloads/Blocker - High JCR Nodes.csv'
    data = {"Blocker - High JCR Nodes": custblockjcrnodes}
    df = pd.DataFrame(data).to_csv(path)
    path = r'/Users/meejain/Downloads/Low hanging fruits.csv'
    data = {"Customers having the max. Probability for their move to CS": lowhangingfruits}
    df = pd.DataFrame(data).to_csv(path)
...
def main():
    ...
    global ans1
    getdatafromSNOW()
    print('Do you want to get the data for entire Basic fleet or just Forms Customers ')
    ans = str(input(""))
    if ans == "y":
        processdata(imsorg,customername,enddate)
    else:
        processdata(imsorgforms,customernameforms,enddateforms)
    displaystats()
    print('Do you want to export the data into csv format ... ')
    ans1 = str(input(""))
    if ans1 == "y":
        exportcsv()
