...
def calculate_otc_stats(sk_otc,host_otc,ss_otc):
    ...
    while not done2:
        ...
        try:
            search_query = 'search=search index=ams_cq ' + 'host=' + host_otc + ' source=*crx-quickstart/logs/oak-tar-compact*.log "*after Compact*" earliest=-1w@w latest=now'
    ...
    if rtest["results"] == []:
        ...
        print ("As per the Offline Compaction Logs in last 4 months, the value after compaction is "+str(floatvalue3))
...
def main():
    ...
    for line in Lines:
        ...
        if rtest["results"] == []:
            ...
            calculate_otc_stats(sk,host,ss)
            print()
            print(
                "############################################################Targetting next instance#####################################################")
            print()
            continue
